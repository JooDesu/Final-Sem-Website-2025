        ...
        mounted() {
            $.ajax({
                ...
                success: async (data) => {
                    ...
                    this.specialBots = Array.isArray(data.specialBots) ? data.specialBots : [];
                    this.squids = Array.isArray(data.squids) ? data.squids : [];
                    this.specialSquids = Array.isArray(data.specialSquids) ? data.specialSquids : [];

                    await nextTick();
                    if (typeof window.initEnemiesPage === "function"
                        // check if the function is defined before calling it
                    ) {
                        window.initEnemiesPage();
                    }
                },
            });
        },
